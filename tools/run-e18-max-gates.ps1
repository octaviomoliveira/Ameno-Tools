[CmdletBinding()]
param(
    [string]$EvidenceDirectory = 'work\e18-gates'
)
...
$tests = @(
    ...
    'tests\maxscript\test_e18_qt_host.ms',
...
$summary.Add('E18 Max 2026 automated regression gate matrix')
$summary.Add('Started: ' + (Get-Date).ToString('o'))
$summary.Add('Legacy WPF UI suites are intentionally excluded; Qt/Python contracts replace those surfaces.')
...
foreach ($relativeTest in $tests) {
    ...
    $listenerText = [System.IO.File]::ReadAllText($listener, [System.Text.Encoding]::UTF8)
...
Write-Host "E18 MAX gates passed: $($tests.Count)/$($tests.Count). Evidence: $evidenceRoot" -ForegroundColor Green
