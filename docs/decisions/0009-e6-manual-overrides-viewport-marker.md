...
- **Schema CA Version 2**:
  Expandir a definição AmenoDimensionCADef (ttribID:#(0x414d454e, 0x44494d31)) para ersion:2 com os campos:
...
- **Marcador Viewport-Only**:
  ...
  - O nó marcador recebe enderable = false e camada AMENO_COTAS. Isso garante que renderizadores de produção (Corona, V-Ray, Arnold, etc.) ignorem o objeto durante a geração de geometria de render.
...
- **Inspeção e Reparo**:
  - inspectDimension valida se uma cota modificada possui seu marcador [M]. Retorna #missing_marker se ausente ou #unexpected_marker se indevido, permitindo reparo automático via epairDimension.
...
- Render limpo garantido por enderable = false e materiais inalterados.
